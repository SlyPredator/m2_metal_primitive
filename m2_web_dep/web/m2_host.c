...
void m2_smooth_step(const double* qpos, const double* qvel, const double* ctrl,
                    double dt, double* qpos_next, double* qvel_next) {
...
static void narrowphase(void) {
    ...
    for (int p=0;p<HC_NPAIR;p++) {
        ...
        } else if (t1==HC_GEOM_PLANE && t2==HC_GEOM_SPHERE) {
            double norm[3]; matcol(mat1,2,norm);
            double dist=(pos2[0]-pos1[0])*norm[0]+(pos2[1]-pos1[1])*norm[1]+(pos2[2]-pos1[2])*norm[2];
            double radius=hc_geom_size[g2*3];
            double cdist=dist-radius;
            if (cdist<inclm) {
                double cpos[3];
                for(int k=0;k<3;k++) cpos[k]=pos2[k]+norm[k]*(-radius - cdist/2);
                emit_con(g1,g2,p,cdist,cpos,norm);
            }
        }
    }
}
...
void m2_full_step(const double* qpos, const double* qvel, const double* ctrl,
...
int main(int argc, char** argv) {
    ...
    for (int t=0;t<h.nsteps;t++) {
        ...
        if (h.scenario==TRAJ_SCEN_AIR) {
            m2_smooth_step(s.qpos, s.qvel, s.ctrl, h.dt, qpn, qvn);
            ...
            m2_full_step(s.qpos, s.qvel, s.ctrl, s.qacc_warmstart, h.dt, newton, qpn, qvn);
            ref_qacc=s.qacc;
        }
